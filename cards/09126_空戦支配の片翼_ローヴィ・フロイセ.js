{
	name: "空戦支配の片翼 ローヴィ・フロイセ",
	cardno: 9126,
	imageno: 6472,
	hp: 7285,
	atk: 3265,
	cost: 55,
	attr: [2,-1],
	species: [8],
	islegend: true,
	ape: "ドルキマスⅠ",
	as1: {
		desc: "<回復・攻撃強化>雷属性の味方のHPを回復(13%)、さらに雷属性の味方の攻撃力をアップ(30%)、SPスキル封印状態時、さらに攻撃力をアップ(90%)",
		proc: [Heal(0.13, [0,0,1,0,0], 0), ChainEnhance(0.3, [0,0,1,0,0], 0)],
	},
	ss1: {
		desc: "<スキルチャージ&遅延>スペシャルスキルの発動ターンを1早め、敵全体の攻撃ターンを1遅らせる",
		turn: 6,
		proc: [ss_skillboost(1), ss_delay_all(1)],
	},
	awakes: [
		Panel_boost([0,0,1,0,0,],2),
		Attr_statusup_oattr(0,400, [0,0,1,0,0,]),
		Attr_statusup_oattr(400,0, [0,0,1,0,0,]),
		Panel_boost([0,0,1,0,0,],2),
		NEFTJOD(30),
		Fastskill(4),
		Attr_statusup(0,400, [0,0,1,0,0,]),
		Attr_statusup(400,0, [0,0,1,0,0,]),
		Awake_noeffect("スキル反射見破り",1),
		Heal_afterbattle(10),
	],
	as2: {
		desc: "<回復・攻撃強化>雷属性の味方のHPを回復(16%)、さらに雷属性の味方の攻撃力をアップ(30%)、SPスキル封印状態時、さらに攻撃力をアップ(90%)",
		proc: [Heal(0.16, [0,0,1,0,0], 0), ChainEnhance(0.3, [0,0,1,0,0], 0)],
	},
	ss2: {
		desc: "<スキルチャージ&遅延>スペシャルスキルの発動ターンを3早め、敵全体の攻撃ターンを2遅らせる",
		turn: 9,
		proc: [ss_skillboost(3), ss_delay_all(2)],
	},
	Lawake: [
		Attr_statusup(0,200, [0,0,1,0,0,]),
		Statusup(1000,0),
	],
}