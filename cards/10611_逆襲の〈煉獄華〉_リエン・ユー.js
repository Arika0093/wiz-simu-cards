{
	name: "逆襲の〈煉獄華〉 リエン・ユー",
	cardno: 10611,
	imageno: 9820,
	hp: 3504,
	atk: 4501,
	cost: 65,
	attr: [1,0],
	species: [8],
	islegend: true,
	ape: "魔轟三鉄傑 対 地獄三十六歌仙",
	as1: {
		desc: "<チェイン攻撃>10チェインでダメージアップ(800%)、20チェインでさらにアップ(700%)",
		proc: [ChainAttack(8.0, 10), ChainAttack(15.0, 20)],
	},
	ss1: {
		desc: "<ステータスアップ>自身の攻撃力とHPを2000アップ(上限値:6000)し、HPを完全に回復する",
		turn: 6,
		proc: [ss_statusup_own([2000, 2000], [6000, 6000], -1), ss_heal_own(1)],
	},
	awakes: [
		Panel_boost([0,1,0,0,0,],4),
		Attr_statusup(200,0, [1,1,0,0,0,]),
		Abstate_invalid("as_sealed"),
		Attr_statusup(0,200, [1,1,0,0,0,]),
		NEFTJOD(30),
		Attr_relief([1,1,1,1,1,],10),
		Abstate_invalid("ss_sealed"),
		Awake_secondfast(4),
		Attr_statusup_sattr(200,0, [0,1,0,0,0,], 200,0, [1,0,0,0,0,]),
		Attr_statusup_sattr(0,200, [0,1,0,0,0,], 0,200, [1,0,0,0,0,]),
	],
	as2: {
		desc: "<チェイン攻撃>10チェインでダメージアップ(900%)、20チェインでさらにアップ(700%)",
		proc: [ChainAttack(9.0, 10), ChainAttack(16.0, 20)],
	},
	ss2: {
		desc: "<ステータスアップ>自身の攻撃力とHPを2000アップ(上限値:6000)し、HPを完全に回復する",
		turn: 6,
		proc: [ss_statusup_own([2000, 2000], [6000, 6000], -1), ss_heal_own(1)],
	},
	Lawake: [
		Statusup(0,500),
		Attr_statusup_sattr(0,200, [0,1,0,0,0,], 0,100, [1,0,0,0,0,]),
	],
}